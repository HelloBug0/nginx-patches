...
本补丁引入了 `ssl_server_name_with_port` 指令。开启后，Nginx 将利用 Proxy Protocol 传递的目标端口信息，尝试构造 `host:port` 格式的 Server Name 进行二次匹配。

## 功能特性
- **新指令**: `ssl_server_name_with_port` (on/off)
- **动态匹配**: 结合 Proxy Protocol 获取的目标端口，支持识别 `Host:Port` 格式的 SNI 请求。
...
3.  **逻辑判断**: 当 SSL 握手发生时，如果 `ssl_server_name_with_port` 为 `on` 且连接包含 Proxy Protocol 信息，代码会将原有的 Host 重新格式化为 `Host:Port`（例如 `example.com` -> `example.com:443`），然后重新在哈希表中查找对应的 Server 配置。

## 注意事项
1.  **宏定义**: 再次强调，编译时若遗漏 `-DNGX_SSL_SERVER_NAME_WITH_PORT`，补丁代码将不起任何作用，且不会报错。
2.  **Proxy Protocol**: 此功能依赖 `c->proxy_protocol->dst_port` 获取端口。如果前端负载均衡器没有开启 Proxy Protocol 发送端口信息，此功能将无法生效。
